...
{\
	if (format == formatWindows || format == formatWindowsXP)\
...
void MIcon::LoadInfoDialog()
{
	...
	AutoEmbedControl(purgeableBox, GetDialogWindow(infoDialog));
	AutoEmbedControl(preloadBox, GetDialogWindow(infoDialog));
	AutoEmbedControl(lockedBox, GetDialogWindow(infoDialog));
	AutoEmbedControl(protectedBox, GetDialogWindow(infoDialog));
	AutoEmbedControl(sysHeapBox, GetDialogWindow(infoDialog));
...
int MIcon::EditIconInfo(int mode)
{
	...
	if (mode == kInsertIcon || mode == kIconInfoBrowser)
	{
		...
		if (formatMenu != NULL)
		{
			...
			DisableMenuItem(formatMenu, formatWindowsXP);
	...
	while (!dialogDone)
	{
		...
		switch (itemHit)
		{
			case iOK:
				...
				if ((temp != ID || mode == kInsertIcon) && file.IsValid())
				{
					Handle otherIcon = NULL;
					short	oldFile, targetFile;
					
					oldFile = CurResFile();
					targetFile = file.OpenResourceFork(fsRdPerm);
					if (targetFile != -1)
					{
						...
						if (otherIcon != NULL)
						{
							...
							itemHit = MUtilities::DisplayAlert(message, NULL, overwriteButtonName, cancelButtonName, (unsigned char*)"\p", kAlertCautionAlert, kWindowAlertPositionParentWindow);
							if (itemHit == 2)
								dialogDone = false;
						}
					}
