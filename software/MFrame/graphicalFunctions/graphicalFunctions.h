...
extern void ChangeContrast(PixMapHandle pix, int contrast, int averageLuminance);
...
extern void ExportGWToQTFile(GWorldPtr imageGW, FSSpec exportFile, OSType creator, OSType fileType, long depth);
extern void ImportQTFileToGWorld(FSSpec file, GWorldPtr target);
extern void GetQTFileInfo(FSSpec file, Rect *fileBounds, int *fileDepth);
...
extern int GetGWorldColors(GWorldPtr gW, Rect bounds, RgnHandle boundingRgn, RGBColor** colors, int** frequencies);
extern int FindInColorTable(CTabHandle colorTable, RGBColor color);

extern void GWorldTransferScaled(GWorldPtr srcGW, Rect srcRect,
						  		 GWorldPtr destGW, Rect destRect,
						  		 RgnHandle maskRegion, bool dithering, int scaling);
extern void PictureTransferScaled(PicHandle srcPic, Rect srcRect,
								  GWorldPtr destGW, Rect destRect,
